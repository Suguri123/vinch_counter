...
4. **📺 대형 모니터 / TV 전광판 & 📱 모바일 QR코드 지원**
   - **📱 스마트폰 접속용 QR코드**: 상단의 `📱` 버튼을 누르면 스마트폰으로 바로 스캔할 수 있는 고화질 QR코드가 팝업되어, 참가자들이 자기 폰으로 주소를 쉽게 열 수 있습니다.
   - **전체화면(Fullscreen) 모드**: 헤더 우측의 `⛶` 버튼을 누르면 브라우저 전체화면으로 전환되어 완벽한 경기장 전광판 모드가 됩니다.
   - **실시간 디지털 시계**: 경기장 분위기를 살리는 초 단위 라이브 시계 내장.
   - **다크 네온 농구/슈팅 코트 디자인**: 눈의 피로를 덜고 행사장에서 시선을 사로잡는 세련된 사이버/네온 스포츠 그래픽.
...
5. **💾 데이터 영구 보존 & 실시간 Firebase 클라우드 연동**
   ...
   - **🔒 관리자 비밀번호 보호 (`1234`)**: 누구나 함부로 기록을 지우지 못하도록, 개별 기록 삭제(`🗑️`) 및 전체 초기화 시 관리자 비밀번호 `1234`를 입력해야만 삭제가 진행됩니다.
